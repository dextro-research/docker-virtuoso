...
# Let's add some other dependencies
RUN apt-get install -y python2.7 python-pip awscli

ADD crontab /etc/cron.d/periodic_back_cron

RUN chmod 0644 /etc/cron.d/periodic_back_cron

#ADD periodic_backup.sh /periodic_backup.sh
#RUN chmod u+x /periodic_backup.sh

RUN touch /var/log/cron.d

ADD dump_one_graph_procedure.sql /dump_one_graph_procedure.sql
